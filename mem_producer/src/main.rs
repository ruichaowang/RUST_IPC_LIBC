...
fn main() {
    ...
    match result {
        Ok(_) => {
            let shm_obj_inner = shm_obj.lock().unwrap().take().unwrap(); // 锁定 Mutex 并获取共享的数据
            // 在这里可以使用 shm_obj_inner 进行后续操作
        },
        Err(_) => {
            println!("Shared memory failed to open");
            drop(shm_obj);
            return;
        },
    }
